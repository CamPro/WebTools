...
{
	function Advertisment($row)
	{
		Module::Module($row);
		if(URL::get('cmd')=='click' and $id=intval(URL::get('id')) and $advertisment = DB::select('advertisment',$id) and $item=DB::select('media','id="'.$advertisment['item_id'].'" and portal_id="'.Portal::$portal_id.'"'))
		{
