					...
					if($file_name and isset($all_items[$setting_id]) and ($all_items[$setting_id]['type']=='FILE' or $all_items[$setting_id]['type']=='IMAGE'))
					{
						...
						if($can_edit)
						{
							...
							else
							{
								...
								if(!is_dir('upload/'.substr(Portal::$portal_id,1).'/block_settings'))
								{
									mkdir('upload/'.substr(Portal::$portal_id,1).'/block_settings');
								}
								$new_file_name = 'upload/'.substr(Portal::$portal_id,1).'/block_settings/'.str_replace('#','_',$block['id']).'_'.time().'_'.$_FILES['items']['name'][$setting_id];
		...
		if($this->get_items($block, $all_items, $item_values))
		{
			...
			$this->parse_layout('list',
				...
				array(
					...
					'copy_setting_id_list'=>array(''=>'')+String::get_list(BlockSettingDB::select_similar_blocks($block))
				)
			);
		}
	}
