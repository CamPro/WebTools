...
)
{
	...
	foreach($items as $key=>$item)
	{
		...
		if($level==0)
		{
			...
			$items[$key]['indent']='<img src="skins/default/images/spacer.gif" width="8" />';
		}
		else
		{
			//if($last['code'] =='133111')
		...
		if($move)
		{
			...
			{
				$items[$key]['move_up']='<a href="'.Url::build_current(array('account_id','type','cmd'=>'move_up','id'=>$item['id'])).'"><i class="fa fa-arrow-up"></i></a>';
				$items[$key]['move_down']='<a href="'.Url::build_current(array('account_id','type','cmd'=>'move_down','id'=>$item['id'])).'"><i class="fa fa-arrow-down"></i></a>';
			}
		}
	}
	if($last)
...
function set_ul_structure($arr,$type=false,$block_id=false,$level=false)
{
	...
	foreach($arr as $value)
	{
		...
		else if(Module::get_setting('url'))
		{
			...
			{
				$href = Url::build(Module::get_setting('url'),array(Module::get_setting('type_params')=>$value['type'],'name_id'=>$value['name_id']),REWRITE);
			}
			else
			{
				$href = Url::build(Module::get_setting('url'),array('name_id'=>$value['name_id']),REWRITE);
			}
		}
		else
		{
			...
			else
			{
				...
				else if(isset($value['type']))				
				{
					...
					$href = Url::build(strtolower($value['type']),array('name_id'=>$value['name_id']),REWRITE);
				}
				else
				{
					$href = Url::build_current(array('name_id'=>$value['name_id']),REWRITE);
				}
			}
		}
		...
		{
			$st .= '<LI>'.(($level==1)?'<i></i>':'').'<a '.($end?' style="border-bottom:0;"':'').'  class="'.(($level==1)?'head'.Module::get_setting('category_type').'':'').' '.(($selected)?'selected':'').'" href="'.$href.'" '.(($type=='ajax')?' onclick="ItemList.blockId = '.$block_id.';Object.extend(ItemList.params,{\'category_id\':'.$value['id'].'});ItemList.GetContent();return false;"':'').'><span>'.strip_tags($value['name']).'</span></a>';
		}
