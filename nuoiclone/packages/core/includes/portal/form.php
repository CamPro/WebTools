...
class Form
{
	...
	function link_css($file_name)
	{
		...
		{
			Portal::$extra_header .= '<link rel="stylesheet" href="'.$file_name.'" type="text/css" />';
		}
	}
	function link_js($file_name)
	{
		...
		{
			Portal::$extra_header .= '<script type="text/javascript" src="'.$file_name.'"></script>';
		}
	}
	...
	function error_messages()
	{
		...
		if($this->error_messages)
		{
			foreach ($this->error_messages as $name=>$error_messages)
			{
				foreach($error_messages as $error_message)
				{
					if(trim($this->name))
					{
						$txt .= '<div><a onclick="var pos=jQuery(\'#'.$name.'\').offset(); window.scrollTo(pos.left,pos.top);jQuery(\'#'.$name.'\').focus().css(\'border\',\'2px inset #ccc\') ;return false;" title="&#7844;n v&#224;o &#273;&#226;y &#273;&#7875; xem v&#7883; tr&#237; x&#7843;y ra l&#7895;i">'.$error_message.'</a></div>';
					}
					else
					{
						$txt .= $error_message;
		...
		$txt .= '<i class="fa fa-times close-error"></i></div>';
		return $txt;
	}
	//In ra cac thong bao loi neu co
