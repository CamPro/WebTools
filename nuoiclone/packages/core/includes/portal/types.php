...
class UsernameType extends NameType
{
	...
	{
		NameType::NameType($required, $messages,4, 64);
	}
	function check($value)
	{
		if(TextType::check($value) && $value!='')
		{
			if(preg_match('/[ÀÁÂÃÈÉÊÌÍÒÓÔÕÙÚÝàáâãèéêìíòóôõùúýĂăĐđĨĩŨũƠơƯưẠ-ỹ.]/',$value))
			{
				$this->error = $this->message;
			}
...
class FloatType extends Type
{
	var $min = 0;
	var $max = 0;
	function FloatType($required=false, $messages=false, $min=0, $max=999999999999999)
	{
