...
<table cellpadding="0" width="100%">
	<tr>
    	<td  class="form_title"><img src="skins/default/images/buttons/<?php echo $action;?>_button.gif" align="absmiddle" alt=""/><?php echo $title;?></td>
		<?php if(URL::get('cmd')=='delete'){?><td class="form_title_button"><a javascript:void(0) onclick="ListPageAdminForm.submit();"><img src="skins/default/images/buttons/delete_button.gif" alt="" width="20" height="20" style="text-align:center"/><br /><?php echo Portal::language('Delete');?></a></td><td class="form_title_button"><a href="<?php echo URL::build_current(array('portal_id','package_id'=>isset($_GET['package_id'])?$_GET['package_id']:'', 'name'=>isset($_GET['name'])?$_GET['name']:''));?>"><img src="skins/default/images/buttons/go_back_button.gif" style="text-align:center" alt=""/><br /><?php echo Portal::language('back');?></a></td><?php }else{ 
		if(User::can_edit()){?><td class="form_title_button"><a href="<?php echo URL::build_current(array('portal_id','package_id','cmd'=>'delete_all_cache'));?>"><img src="skins/default/images/buttons/delete_cache_button.gif" style="text-align:center"/><br /><?php echo Portal::language('delete_cache');?></a></td><?php } 
		if(User::can_add()){?><td class="form_title_button"><a href="<?php echo URL::build_current(array('portal_id','package_id'=>isset($_GET['package_id'])?$_GET['package_id']:'', 'name'=>isset($_GET['name'])?$_GET['name']:'')+array('cmd'=>'add'));?>"><img src="skins/default/images/buttons/add_button.gif" style="text-align:center" alt=""/><br /><?php echo Portal::language('Add');?></a></td><td class="form_title_button" align="center"><a href="<?php echo URL::build_current(array('portal_id','package_id'=>isset($_GET['package_id'])?$_GET['package_id']:'', 'name'=>isset($_GET['name'])?$_GET['name']:'')+array('cmd'=>'add_blocks'));?>"><img src="skins/default/images/buttons/add.jpg" style="text-align:center" alt=""/><br /><?php echo Portal::language('Add_block');?></a></td><?php }?><?php if(User::can_delete()){?>
        <td class="form_title_button">
				<a javascript:void(0) onclick="ListPageAdminForm.cmd.value='delete';ListPageAdminForm.submit();"><img src="skins/default/images/buttons/delete_button.gif" alt=""/><br /><?php echo Portal::language('Delete');?></a>
            </td><?php }}?>
            <td class="form_title_button">
            <a  href="<?php echo URL::build('default');?>"><img src="skins/default/images/buttons/frontpage.gif" alt=""/><br />Trang ch&#7911;</a></td></tr></table>
	...
	<tr bgcolor="#EFEFEF" valign="top">
		<td nowrap bgcolor="#EFEFEF">
			...
			<?php
					if(isset($this->map['packages']) and is_array($this->map['packages']))
					{
						foreach($this->map['packages'] as $key1=>&$item1)
						{
							if($key1!='current')
							{
								$this->map['packages']['current'] = &$item1;?>
			<tr><td nowrap>
				<a href="<?php echo URL::build_current(array('portal_id'));?>&package_id=<?php echo $this->map['packages']['current']['id'];?>" class="home-news-category-level<?php if(URL::get('package_id')==$this->map['packages']['current']['id'])echo '_selected'.$this->map['packages']['current']['level'];else echo $this->map['packages']['current']['level'];?>"><?php echo $this->map['packages']['current']['name'];?></a>
					...
					unset($this->map['packages']['current']);
					} ?>
			</table>
				...
				<td width="100%">
					...
					if(isset($this->map['items']) and is_array($this->map['items']))
					{
						...
						<tr bgcolor="<?php if((URL::get('just_edited_id',0)==$this->map['items']['current']['id']) or (is_numeric(array_search($this->map['items']['current']['id'],$this->map['just_edited_ids'])))){ echo '#EFFFDF';} else {echo $this->map['items']['current']['is_sibling']?'#FFFFDF':'white';}?>" valign="middle" <?php Draw::hover('#E2F1DF');?> style="cursor:pointer;" id="PageAdmin_tr_<?php echo $this->map['items']['current']['id'];?>">
							...
							<td><a target="_blank" href="<?php echo URL::build($this->map['items']['current']['name']);?>&<?php echo $this->map['items']['current']['params'];?>"><img src="skins/default/images/buttons/select.jpg" /></a></td>
	...
	'name'=>isset($_GET['name'])?$_GET['name']:'',    
	  )+array('cmd'=>'edit','id'=>$this->map['items']['current']['id'])); ?>"><img src="skins/default/images/buttons/edit.gif" alt="<?php echo Portal::language('Edit');?>" width="12" height="12" border="0"></a></td>
	  						<td nowrap><a href="<?php echo Url::build_current(array('portal_id','package_id', 'name')+array('cmd'=>'duplicate','id'=>$this->map['items']['current']['id'])); ?>"><img src="skins/default/images/buttons/duplicate.gif" alt="<?php echo Portal::language('duplicate');?>" width="13" height="13" border="0"></a></td>
							<?php
							}
							?>						</tr>
...
<?php 
			...
			<td width="100%">
				...
				<a javascript:void(0) onclick="select_all_checkbox(document.ListPageAdminForm,'PageAdmin',true,'#FFFFEC','white');"><?php echo Portal::language('select_all');?></a>&nbsp;
				<a javascript:void(0) onclick="select_all_checkbox(document.ListPageAdminForm,'PageAdmin',false,'#FFFFEC','white');"><?php echo Portal::language('select_none');?></a>
				<a javascript:void(0) onclick="select_all_checkbox(document.ListPageAdminForm,'PageAdmin',-1,'#FFFFEC','white');"><?php echo Portal::language('select_invert');?></a>
			</td>
			<td>
				<a name="bottom_anchor" javascript:void(0)><img src="skins/default/images/top.gif" title="<?php echo Portal::language('top');?>" border="0" alt="<?php echo Portal::language('top');?>"></a>
			</td>
			</tr></table>
