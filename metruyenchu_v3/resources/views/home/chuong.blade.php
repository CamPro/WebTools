    ...
    <div id="body-truyen" class="container-fluid myboder-2 shadow-sm" {{ returnCookiePage() }}>
        ...
        <div class="content-truyen">
            <div class="phan-trang mb-3">
                ...
                <a class="btn my-btn-luot btn-phai float-right btn-outline-secondary @if($chuong->numchap >= $truyen->num_chaps - $truyen->num_chaps_pending) disabled @endif" href="{{ route('trangchu.chuong', ['truyen'=>$truyen->name_slug, 'chuong'=>'chuong-'.($chuong->numchap+1)]) }}">Tiếp →</a>
            </div>
                ...
                @else
                    ...
                    @else
                        @if(getIdUser() > 0)
                            ...
                            {!! $chuong->content !!}
                            @else
                                @if(getCoin() >= $chuong->coin)
                                <div class="khoa-chuong text-center">
                                    <i class="fas fa-sack-dollar fa-4x text-danger"></i>
                                    <p class="mt-2">Bạn cần {{ $chuong->coin }} linh thạch để mở khóa chương.<br/>
                                        <small>Chú ý: Đăng truyện để có thêm linh thạch.</small>
                                    </p>
                                    <a href="{{ route('member.mua_chuong', ['id' => $chuong->id]) }}" class="btn btn-outline-primary">
                                        <i class="fas fa-unlock-alt"></i> Mở khóa chương
                                    </a>
                                </div>
                                @else
                                <div class="khoa-chuong text-center">
                                    <p class="mt-2">Bạn không đủ {{ $chuong->coin }} linh thạch để mở khóa chương.<br/>
                                        <small>Chú ý: Đăng truyện để có thêm linh thạch.</small>
                                    </p>
                                    <a href="{{ route('member.naplinhthach') }}" class="btn btn-outline-success">
                                    <i class="fas fa-sack-dollar"></i> Mua linh thạch
                                    </a>
                                </div>
                                @endif
                            @endif
                        ...
                        <div class="khoa-chuong text-center">
                            <i class="fas fa-sack-dollar fa-4x text-danger"></i>
                            <p class="mt-2">Đăng nhập để mở khóa chương.</p>
                        </div>
                        @endif
                    @endif
                @endif
