        ...
        <div class="row">
            ...
            <div class="col-md-9">
                ...
                <div class="info-thong-so text-center mt-3 hide-pc">
                    ...
                    <span class="my-btn my-1 badge badge-success font-weight-bolder my-gage ml-2">{{ number_format($truyen->num_chaps - $truyen->num_chaps_pending) }} chương</span>
                ...
                <div class="ngay-tao mt-3">
                    <p class="py-0 my-0">
                        <i class="fas fa-calendar-alt"></i> Xuất bản: {{ $truyen->created_at->format('d-m-Y H:i:s') }}
                    </p>
                    <p class="py-0 my-0">
                    ...
                    <ul id="list-chaps">
                        ...
                        <li>
                            <a href="{{ route('trangchu.chuong', ['truyen'=>$truyen->name_slug, 'chuong'=>'chuong-'.$val->numchap]) }}">Chương {{ $val->numchap }}: {{ $val->name }}
                                @if($val->lock == 'Y')
                                <span class="float-right"><i class="fas fa-sack-dollar"></i></span>
                                @endif
                            </a>
                        </li>
                        @endforeach
...
@if($truyen['num_chaps'] > 25)
<div class="my-pagination mt-4 text-center my-3">
    <a id="btn-fist" href="{{ route('trangchu.truyen', ['name_slug' => $truyen['name_slug']]) }}?page=1" class="btn {{ disibleButtonPrev($page) }}"><i class="fad fa-backward"></i></a>
    <a id="btn-truoc" href="{{ route('trangchu.truyen', ['name_slug' => $truyen['name_slug']]) }}?page={{ ($page - 1) }}" class="btn btn-outline-secondary btn-trang {{ disibleButtonPrev($page) }}"><i class="fas fa-caret-left"></i> Trước</a>

    <span id="page-of" class="btn btn-outline-secondary btn-page-of disabled ml-2"><span id="currPage">{{ $page }}</span>/{{ $numOfPage }}</span>

    <a id="btn-sau" href="{{ route('trangchu.truyen', ['name_slug' => $truyen['name_slug']]) }}?page={{ ($page + 1) }}" class="btn btn-outline-secondary btn-trang ml-2 {{ disibleButtonNext($page, $numOfPage) }}">Tiếp <i class="fas fa-caret-right"></i></a>
    <a id="btn-tail" href="{{ route('trangchu.truyen', ['name_slug' => $truyen['name_slug']]) }}?page={{ $numOfPage }}" class="btn {{ disibleButtonNext($page, $numOfPage) }}"><i class="fad fa-forward"></i></a>
</div>
@endif
        ...
        <div class="col-md-8">
            <div class="khung-binh-luan-truyen">
                <div style="position: relative;">
                    ...
                    <div class="fream-img" style="width: 75px; float: left; margin-right: 5px;"><img src="{{ checkAvatar() }}" width="60" class="mr-3"></div>
            ...
            <div class="frame-chua-binh-luan mt-3">
                <div id="append-comment">
                    <!-- start loop -->
                    @foreach($binhluan as $val)
                    <div id="cmt-{{ $val->id }}" class="card card-mobile">
                        <div class="card-header px-2 car-header">
                            <a class="frame-image-thum" href="{{ route('trangchu.getPublicInfoUser', ['id' => $val->user_id]) }}">
                                <div class="float-left">
                                    <img src="{{ getAvatar($val->User->avatar) }}" width="50" class="mx-auto rounded-circle">
                                </div>
                                <div class="float-left pl-3">
                                    <a href="{{ route('trangchu.getPublicInfoUser', ['id' => $val->user_id]) }}" class="username">{{ $val->User->display_name }}</a><br>
                                    <span class="userTitle">{{ $val->User->exp_level }}</span>
                                </div>
                            </a>
                        ...
                        <div class="card-body py-0">
                            <div class="message-attribution">
                                ...
                                <div class="float-right user-reply">
                                    #Chương {{ $val->chap }}&ensp;
                                    <span><i class="fas fa-reply"></i> Trả lời</span>
                                    @if(getIdUser() > 0)
                                        @if(getIdUser() == $val->user_id)
                                            &ensp;
                                            <span onclick="deleteComment(`{{ $val->id }}`)" class="cmt-check">
                ...
                <div class="text-center pt-3 pb-1">
                    <button class="btn btn-primary">Xem thêm bình luận</button>
                </div>
            </div>
        </div>
                        ...
                        <div class="media-body">
                            <h5 class="mt-0 mb-1">{{ $val->User->display_name }}
                                <div class="float-right hide-mobile">
                                    @for($num_binhluan = 0; $num_binhluan < 5; $num_binhluan++) @if($num_binhluan < $val->marks)
...
<script type="text/javascript">
            ...
            if (truyen_comment.length <= 0) {
                ...
                $.ajax({
                    ...
                    success: function(data_return) {
                        $('#append-comment').prepend(data_return);
    ...
    function deleteComment(_id) {
        $.ajax({
            url: "{{ route('trangchu.xoa_binh_luan') }}",
            data: {
                id: _id,
                _token: _token
            },
