...
@section('title', 'Tủ Sách | Mê Truyện Chữ')
@section('content')
<div class="my-card card shadow-sm">
    <h5 class="card-header font-roman py-2 font-weight-bolder my-t-shadow bg-xanh">
        <i class="far fa-stopwatch"></i> TỦ SÁCH
        <span class="float-right bgg-trang">{{ $totalTruyen }}</span>
    </h5>
    ...
    <div class="card-footer">
        <div class="my-pagination text-center">
            <a id="btn-fist" href="{{ route('trangchu.tu_sach') }}?page=1" class="btn {{ disibleButtonPrev($page) }}"><i class="fad fa-backward"></i></a>
            <a id="btn-truoc" href="{{ route('trangchu.tu_sach') }}?page={{ ($page-1) }}" class="btn btn-outline-secondary btn-trang {{ disibleButtonPrev($page) }}"><i class="fas fa-caret-left"></i> Trước</a>

            <span id="page-of" class="btn btn-outline-secondary btn-page-of disabled ml-2"><span id="currPage">{{ $page }}</span>/{{ $numOfPage }}</span>

            <a id="btn-sau" href="{{ route('trangchu.tu_sach') }}?page={{ ($page+1) }}" class="btn btn-outline-secondary btn-trang ml-2 {{ disibleButtonNext($page, $numOfPage) }}">Tiếp <i class="fas fa-caret-right"></i></a>
            <a id="btn-tail" href="{{ route('trangchu.tu_sach') }}?page={{ $numOfPage }}" class="btn {{ disibleButtonNext($page, $numOfPage) }}"><i class="fad fa-forward"></i></a>
        </div>
    </div>
</div>
