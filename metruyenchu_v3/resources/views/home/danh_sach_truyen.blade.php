...
<div class="my-card card shadow-sm">
    ...
    <div class="card-footer">
        <div class="my-pagination text-center">
            <a id="btn-fist" href="{{ route('trangchu.danh_sach_truyen') }}?page=1" class="btn {{ disibleButtonPrev($page) }}"><i class="fad fa-backward"></i></a>
            <a id="btn-truoc" href="{{ route('trangchu.danh_sach_truyen') }}?page={{ ($page-1) }}" class="btn btn-outline-secondary btn-trang {{ disibleButtonPrev($page) }}"><i class="fas fa-caret-left"></i> Trước</a>

            <span id="page-of" class="btn btn-outline-secondary btn-page-of disabled ml-2"><span id="currPage">{{ $page }}</span>/{{ $numOfPage }}</span>

            <a id="btn-sau" href="{{ route('trangchu.danh_sach_truyen') }}?page={{ ($page+1) }}" class="btn btn-outline-secondary btn-trang ml-2 {{ disibleButtonNext($page, $numOfPage) }}">Tiếp <i class="fas fa-caret-right"></i></a>
            <a id="btn-tail" href="{{ route('trangchu.danh_sach_truyen') }}?page={{ $numOfPage }}" class="btn {{ disibleButtonNext($page, $numOfPage) }}"><i class="fad fa-forward"></i></a>
        </div>
    </div>
</div>
