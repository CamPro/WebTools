        ...
        <div class="section-block mt-0 pt-0">
            <div class="d-flex justify-content-between align-items-center">
                <h1 class="section-title mb-0">
                    <a href="{{ route('member.dashboard.my_story.list_chapter.add_chap', ['name_slug'=>$truyen->name_slug]) }}" class="btn btn-primary {{ statusCheck($truyen['status']) }}">
                        ...
                        <table class="table table-hover">
                            ...
                            <tbody>
                                ...
                                <tr id="chuong-{{ $val->id }}">
                                    ...
                                    <td class="align-middle">
                                        @if($val->lock == 'Y')
                                        <span id="check-{{ $val->id }}" style="color: red;"><i id="icon-lock-{{ $val->id }}" class="fas fa-sack-dollar"></i> Vào VIP</span>
                                        @else
                                        <span id="check-{{ $val->id }}"><i id="icon-lock-{{ $val->id }}" class="fas fa-sack"></i> Miễn Phí</span>
                                        @endif
                                    </td>
                                    <td id="coin-{{ $val->id }}" class="align-middle">{{ $val->coin }}</td>
                                    ...
                                    <td class="align-middle text-right">
                                        ...
                                        <button class="btn btn-sm btn-icon btn-secondary" onclick="RequestLockChap('{{ $val->id }}', '{{ $val->name }}')" title="Vào VIP/Thoát VIP">
...
<script>
    ...
    function Confirm_Lock(id, name) {
        // $.confirm({
        //     title: '<i class="fas fa-rocket"></i> Thông Báo!',
        //     content: 'Bạn có muốn chương [<b>' + name + '</b>] vào VIP?',
        //     type: 'purple',
        //     buttons: {
        //         deleteUser: {
        //             text: '<i class="fas fa-sack-dollar"></i> VIP',
        //             btnClass: 'btn-red',
        //             action: function() {
        //                 $.get("{{ route('member.lock_chapter') }}", {
        //                     id: id
        //                 }, function() {
        //                     $('#check-' + id).html('<i id="icon-lock-' + id + '" class="fas fa-sack-dollar"></i> Vào VIP');
        //                     $('#check-' + id).css('color', 'red');
        //                     toastr.success('Vào VIP thành công!', '');
        //                 });
        //             }
        //         },
        //         calcelAction: {
        ...
        $.confirm({
            ...
            content: '' +
                ...
                '<label>Giá Chương VIP (*)</label>' +
                '<input type="number" class="form-control" id="form-input-coin" min="1" step="1" value="15" placeholder="Giá mở khóa chương">' +
                '<small class="form-text text-muted">Vào VIP chương: '+name+'</small>' +
                '</div>' +
                '</form>',
            type: 'orange',
            buttons: {
                formSubmit: {
                    ...
                    btnClass: 'btn-blue',
                    action: function() {
                        var my_coin = this.$content.find('#form-input-coin').val();
                        $.ajax({
                            url: "{{ route('member.lock_chapter') }}",
                            data: {
                                id: id,
                                coin: my_coin
                            },
    ...
    function Confirm_UnLock(id, name) {
        $.confirm({
            title: '<i class="fas fa-rocket"></i> Thông Báo!',
            content: 'Bạn có muốn chương [<b>' + name + '</b>] thoát khỏi VIP?',
            type: 'purple',
            buttons: {
                deleteUser: {
                    text: '<i class="fas fa-sack"></i> Thoát VIP',
                    btnClass: 'btn-red',
                    action: function() {
                        $.get("{{ route('member.un_lock_chapter') }}", {
                            id: id
                        }, function() {
                            $('#check-' + id).html('<i id="icon-lock-' + id + '" class="fas fa-sack"></i> Miễn Phí');
                            $('#check-' + id).css('color', 'black');
                            $('#coin-' + id).text('0');
                            toastr.success('Thoát VIP thành công!', '');
                        });
                    }
                },
                calcelAction: {
