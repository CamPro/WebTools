...
function getAvatarById($id){
    $user = User::find($id);
    return asset('storage/app/public/avatar').'/'.$user['avatar'];
}
...
function disibleButtonPrev($current_page){
    if($current_page <= 1){
        echo 'disabled';
    }
}

function disibleButtonNext($current_page, $max_page){
...
function statusCheck($status){
    if($status == 'Hoàn thành' || $status == 'Ngừng'){
        echo 'disabled'; 
    }
}
...
function clearHTML_DTruyen($string){
    $resutl = '';
    ...
    $resutl = (string) $resutl;

    // $arr = explode(' ', trim($resutl));
...
function getAvatar_ThanhVien($user_id){
...
function showSelectType($types, $child){
...
function getStoryNameByChapterId($chapter_id){
    $chapter = ChuongController::getDetailChap($chapter_id);
