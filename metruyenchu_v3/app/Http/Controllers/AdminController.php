...
class AdminController extends Controller{
    ...
    public function getDanhSachTruyen(){
        $truyen = TruyenController::getPaginate(25);
    ...
    public function getNapTienTrucTuyen(){
        $hoadon = ThanhToanController::getAll();
        return view('admin.naplinhthach', compact('hoadon'));
    }

    public function getUpdateTrangThaiHoaDon($id){
        ThanhToanController::checkBill($id);
        return redirect()->route('admin.naptien');
    }

    public function getCancelNapTien($id){
        ThanhToanController::cancelBill($id);
        return redirect()->route('admin.naptien');
    }

    public function getUpdateTruyenVip($id){
        TruyenController::updateVIP($id);
        return redirect()->route('admin.stories'); 
    }

    public function ajaxSearchTruyen(Request $res){
        $truyen = TruyenController::searchTruyen($res->key);
        $data_return = '';

        foreach($truyen as $val){
            $data_return .= '<tr>
            <td class="text-center font-weight-bold align-middle" width="15">'.$val->id.'</td>
                <td>
            ...
            if($val->lock == 'N'){
                $data_return .= '<td class="align-middle"><span class="badge badge-warning">Không</span></td>';
            }
            else{
                $data_return .= '<td class="align-middle"><span class="badge badge-warning">Đã Khóa</span></td>';
            }

            if($val->delete == 'Y'){
                $data_return .= '<td class="align-middle"><span class="badge badge-info">Đã Xóa</span></td>';
            }
            else{
                $data_return .= '<td class="align-middle"><span class="badge badge-info">Không</span></td>';
            }

            if($val->vip == 'Y'){
                $data_return .= '<td class="align-middle">
                    <a class="btn btn-sm btn-icon btn-secondary mr-2" href="'.route('admin.truyen.vip', ['id'=>$val->id]).'">
            ...
            $data_return .= ' <td class="align-middle text-right">
