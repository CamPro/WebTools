...
class ThanhVienController extends Controller{
    public function getThanhVien_Truyen(Request $request){
        ...
        foreach($thanhvien as $val){
            $data .= '<div class="col-md-1 mb-2 position-relative">
                <span class="position-absolute" style="top: 3px; right: 20px; z-index: 999; cursor: pointer;">
                    <i class="fas fa-times-square" onclick="deleteMember(`'.$val->id.'`)" style="font-size: 20px;"></i>
                </span>
                <div class="user-avatar user-avatar-xl fileinput-button rounded mx-auto d-block">
                    <img src="'.getAvatar_ThanhVien($val->user_id).'">
                </div>
                <p class="text-center">'.$val->User->display_name.'<p/>
            </div>';
        }
