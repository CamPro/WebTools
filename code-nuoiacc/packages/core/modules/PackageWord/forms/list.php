...
class ListPackageWordForm extends Form
{
	...
	function on_submit()
	{
		if($_REQUEST['records'])
		{
			foreach($_REQUEST['records'] as $id=>$record)
			{
				if($record['id'])
				{
	...
	function draw()
	{
		$languages = Portal::template_cache('language');
		if(URL::get('order_by'))
		{
		...
		$item_per_page = isset($module)?200:Module::$current->get_setting('item_per_page',50);
		DB::query('
