...
class ListPageAdminForm extends Form
{
	function ListPageAdminForm()
	{
		...
		$this->link_css('skins/default/css/category.css');
	}
	function on_submit()
	{
		if(URL::get('confirm'))
		{
			...
			Url::redirect_current(array('portal_id','package_id','name','cmd'));
		}
	}
	function draw()
	{
		...
		$cond = ' 1 '
			...
			.(URL::get('portal_id')?
				...
				' and params like "'.URL::get('portal_id').'%"'):'')
		;
		...
		DB::query('
			...
			where '.$cond.'
		');
		$count = mysql_num_rows(DB::$db_result);
		...
		DB::query('
			...
			where '.$cond.'
			group by page.name
			'.(URL::get('order_by')?'order by '.URL::get('order_by').(URL::get('order_dir')?' '.URL::get('order_dir'):''):'order by package_id, page.name').'
