...
class IDStructure
{
 	...
 	function parent($structure_id)
	{
		...
		else
		{
			...
			//$structure_id=bcadd($structure_id,0);
	...
	{
		//return bcadd($structure_id,bcpow(ID_BASE,ID_MAX_LEVEL - IDStructure::level($structure_id)));
	...
	function path_cond($structure_id)
	{
		...
		{
			$path .= ','.number_format($structure_id,0,'','').'';;
		}
	...
	{
		$level = IDStructure::level($structure_id);
		//$child_offset = bcadd(bcpow(ID_BASE, ID_MAX_LEVEL-($level+$child_level),0),0);
