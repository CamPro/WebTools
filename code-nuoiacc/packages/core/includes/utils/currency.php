...
// doc gia tien tieng viet	
	...
	{
		$unit = array(1=>'',2=>'nghìn',3=> 'triệu',4=> 'tỉ',5=>'nghìn',6=>'triệu',7=>'tỉ');
		$text = '';
		$i = 1;
	...
	function read_currency($number)
	{
		...
		//if($number==562) echo $temp[3].$temp[2].$temp[1];
...
// ket thuc doc gia tien tieng viet
	//Hien thi ngay thang nam cuoi trang in
	...
	function exchange_rate($price,$current_exchange,$to_exchange,$currency = false,$label = true)
	{
		if($price==0)
		{
			return Portal::language('contact');
		}		
		$new_price = $price;
		if(!$currency)
		{
			$currency = Portal::template_cache('currency');
		}
		if(($current_exchange!=$to_exchange) && (isset($currency[$current_exchange])) && (isset($currency[$to_exchange])))
		{
			$new_price = ($currency[$current_exchange]['exchange']*$price)/$currency[$to_exchange]['exchange'];
		}
		if($label)
		{
			return System::display_number($new_price).($label?' '.($to_exchange?$to_exchange:$current_exchange):'');
		}
		return $new_price;
	}
?>