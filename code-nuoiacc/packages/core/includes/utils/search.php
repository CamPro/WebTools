...
function hightlight_keyword($source, $keywords,$fulltext = true)
{
	if($keywords)
	{
		if($fulltext)
		{
			...
			{
				if(strlen($value)<7 and strlen($value)>=2)
				{
					$value = str_replace(array('/'),array('\/'),$value);				
					$source = preg_replace(array("/([\w\sa-zA-Z\,\.\'\"\?\!]+)($value)([\w\sa-zA-Z\,\.\'\"\?\!]+)/i"), array("\\1<strong><span style='background-color:yellow;color:black'>\\2</span></strong>\\3"), $source);				
				}
				else
				{
					$source = str_replace($value,'<strong><span style=\'background-color:yellow;color:black\'>'.$value.'</span></strong>',$source);
				}
