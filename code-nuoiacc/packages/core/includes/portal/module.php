...
class Module
{
	...
	function on_draw()
	{
		...
		if(User::is_admin() and !$this->data['module']['use_dblclick'] and !defined('XPATH'))
		{
			echo '<script type="text/javascript">make_module_title('.$this->data['module']['id'].',"'.$this->data['module']['name'].'","'.$this->data['module']['type'].'",'.$this->data['id'].',"'.$this->data['region'].'","'.$this->data['page_id'].'","'.Portal::$page_gen_time->get_timer().'","'.$this->data['container_id'].'","'.$this->data['module']['path'].'");</script>';
		}
		$this->draw();
		if(User::is_admin() and !$this->data['module']['use_dblclick'] and !defined('XPATH'))
		{
