...
class GenerateLayout
{
	...
	function synchronize()
	{
		...
		return preg_replace(array(
			...
			'<input  name="\\1" type ="checkbox" <?php if(URL::get(\'\\1\')){echo "checked=\"checked\"";}?>/>',
		...
		while(($pos=strpos($text,'<select name="'))!==false)
		{
			if($pos2 = strpos($text,'"',$pos+15))
			{
				...
				if($items = $this->get_text_variable($var.'_list'))
				{
					...
					?><?php if(isset($_REQUEST[\''.$var.'\']) or isset($this->map[\''.$var.'\'])){?><script type="text/javascript">document.getElementById(\''.$var.'\').value = "<?php echo addslashes(URL::get(\''.$var.'\',isset($this->map[\''.$var.'\'])?$this->map[\''.$var.'\']:\'\'));?>";</script><?php }?>';
