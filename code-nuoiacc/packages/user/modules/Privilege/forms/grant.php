...
class GrantPrivilegeForm extends Form
{
	...
	function draw()
	{
		$privilege_id_list = String::get_list(DB::fetch_all('
			select 
				privilege.id,
				privilege.title_'.Portal::language().' as name
			from 
				privilege
			order by name')); 
		...
		if($privilege = DB::select('privilege',$privilege_id))
		{
			...
			$modules = DB::fetch_all('
				...
				from 
					...
					left outer join privilege_module on module.id=module_id and privilege_id="'.URL::get('id').'"
				where
