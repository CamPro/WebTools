...
class FrameForm extends Form
{
	...
	function draw()
	{
		...
		{
			if(Url::get('name_id') and $category = DB::fetch('select id,name_'.Portal::language().' as name,name_id from category where name_id="'.Url::sget('name_id').'" and category.portal_id="'.Portal::$portal_id.'"'))
			{
				$title = $category['name'];
			}
			elseif(Url::iget('category_id') && ($category = DB::fetch('select id,name_id,name_'.Portal::language().' as name from category where portal_id="'.Portal::$portal_id.'" and id='.Url::iget('category_id'))))
			{
				$title = $category['name'];				
			}
			else
			{
				$title = Module::get_setting('title');
			}
		}
		else
		{
