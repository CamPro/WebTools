...
//	DATE  : 18/01/2011
class AutoReaction extends Module
{
	function AutoReaction($row)
	{
		Module::Module($row);
		require_once 'db.php';
		switch(Url::get('cmd'))
		{
			...
			default:
				...
				$this->add_form(new AutoReactionForm());
				break;
		}
	}
	...
	function save_schedules()
	{
		if(isset($_REQUEST['facebook_id']) and isset($_REQUEST['type']) and $_REQUEST['type'] and ($account = DB::fetch('select * from facebook_accounts where id='.Url::iget('facebook_id'))))
		{
			...
			if(!DB::fetch('select id from facebook_bot where account_id="'.$_REQUEST['facebook_id'].'" and type="1"' ))
			{
