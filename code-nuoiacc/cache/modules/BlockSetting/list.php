	...
	<div>
		...
		<form name="CopyBlockSettingForm" method="post" action="?<?php echo htmlentities($_SERVER['QUERY_STRING']);?>">
			...
			<table><tr><td width="150"><?php echo Portal::language('copy_from');?></td><td> <select  name="copy_setting_id"><?php
					...
					?><?php if(isset($_REQUEST['copy_setting_id']) or isset($this->map['copy_setting_id'])){?><script type="text/javascript">document.getElementById('copy_setting_id').value = "<?php echo addslashes(URL::get('copy_setting_id',isset($this->map['copy_setting_id'])?$this->map['copy_setting_id']:''));?>";</script><?php }?></select> <input type="submit" name="copy_setting" value="  <?php echo Portal::language('copy');?>  "/></td></tr></table>
