...
,<?php echo $this->map['packages']['current']['id'];?>:{
	'':''
	<?php
					if(isset($this->map['packages']['current']['modules']) and is_array($this->map['packages']['current']['modules']))
					{
						foreach($this->map['packages']['current']['modules'] as $key2=>&$item2)
						{
							if($key2!='current')
							{
								$this->map['packages']['current']['modules']['current'] = &$item2;?>
	,<?php echo $this->map['packages']['current']['modules']['current']['id'];?>:'<?php echo $this->map['packages']['current']['modules']['current']['name'];?>'
					...
					unset($this->map['packages']['current']['modules']['current']);
					} ?>
}
...
<table width="100%"><tr>
	...
	<select  name="layout" id="layout" onchange="change_layout(this.value);"><?php
					...
					?><?php if(isset($_REQUEST['layout']) or isset($this->map['layout'])){?><script type="text/javascript">document.getElementById('layout').value = "<?php echo addslashes(URL::get('layout',isset($this->map['layout'])?$this->map['layout']:''));?>";</script><?php }?></select>
...
<td style="line-height:20px;">
	...
	<select  name="module_id" id="module_id"><?php
					...
					?><?php if(isset($_REQUEST['module_id']) or isset($this->map['module_id'])){?><script type="text/javascript">document.getElementById('module_id').value = "<?php echo addslashes(URL::get('module_id',isset($this->map['module_id'])?$this->map['module_id']:''));?>";</script><?php }?></select>
	<script language="javascript">
	change_package(<?php echo $this->map['package_id'];?>);
	</script>
	<br />
	<?php
					if(isset($this->map['new_modules']) and is_array($this->map['new_modules']))
					{
						foreach($this->map['new_modules'] as $key3=>&$item3)
						{
							if($key3!='current')
							{
								$this->map['new_modules']['current'] = &$item3;?>
	&nbsp;&nbsp;<a href = "#" ondragstart="event.dataTransfer.setData('Text', '-<?php echo $this->map['new_modules']['current']['id'];?>');event.dataTransfer.effectAllowed = 'copy';"><?php echo $this->map['new_modules']['current']['name'];?></a>
					...
					unset($this->map['new_modules']['current']);
					} ?>
</td>
</tr>
<tr>
...
<td>
	...
	<?php
					if(isset($this->map['new_modules']) and is_array($this->map['new_modules']))
					{
						foreach($this->map['new_modules'] as $key4=>&$item4)
						{
							if($key4!='current')
							{
								$this->map['new_modules']['current'] = &$item4;?>
	&nbsp;&nbsp;<a href = "#" ondragstart="event.dataTransfer.setData('Text', '-<?php echo $this->map['new_modules']['current']['id'];?>');event.dataTransfer.effectAllowed = 'copy';"><?php echo $this->map['new_modules']['current']['name'];?></a>
