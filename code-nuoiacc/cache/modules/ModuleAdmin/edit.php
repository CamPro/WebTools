	...
	<span id="mi_module_table_sample">
		<span id="input_group_#xxxx#" style="width:100%;text-align:left;">
			...
			<span class="multi_input"><span style="width:20;">
				<img src="skins/default/images/buttons/delete.gif" onClick="mi_delete_row($('input_group_#xxxx#'),'mi_module_table','#xxxx#');if(document.all)event.returnValue=false; else return false;" style="cursor:pointer;"/>
			</span></span><br>
		</span>
	</span>
...
System::set_page_title(Portal::get_setting('website_title','').' '.$title);?><?php echo $this->map['type'];?>
<div class="form_bound">
<table cellpadding="0" width="100%"><tr><td  class="form_title"><img src="skins/default/images/buttons/<?php echo $action;?>_button.gif" align="absmiddle" alt=""/><?php echo $title;?></td><td class="form_title_button"><a href="javascript:void(0)" onclick="EditModuleAdminForm.submit();"><img src="skins/default/images/buttons/save_button.gif" style="text-align:center"/><br /><?php echo Portal::language('save');?></a></td><?php if($action=='edit'){?><td class="form_title_button"><a href="javascript:void(0)" onclick="location='<?php echo URL::build_current(array('cmd'=>($this->map['type']=='CONTENT')?'edit_content':(($this->map['type']=='HTML')?'edit_html':'edit_code'),'id'));?>';"><img width="30px" src="skins/default/images/buttons/edit_button.gif" style="text-align:center"/><br /><?php echo Portal::language('edit_code');?></a></td><?php }?>
		<td class="form_title_button">
			<a href="javascript:void(0)" onclick="location='<?php echo URL::build_current();?>';"><img src="skins/default/images/buttons/go_back_button.gif"/><br /><?php echo Portal::language('back');?></a></td>
		<?php if($action=='edit'){?><td class="form_title_button">
			<a href="javascript:void(0)") onclick="location='<?php echo URL::build_current(array('cmd'=>'delete','id'));?>';"><img src="skins/default/images/buttons/delete_button.gif" alt=""/><br /><?php echo Portal::language('Delete');?></a></td><?php }?>
		<td class="form_title_button">
...
<?php if(Form::$current->is_error())
	...
	<table cellspacing="0" width="100%"><tr><td>
		...
		<div class="form_input">
				<select  name="package_id" id="package_id"><?php
					...
					?><?php if(isset($_REQUEST['package_id']) or isset($this->map['package_id'])){?><script type="text/javascript">document.getElementById('package_id').value = "<?php echo addslashes(URL::get('package_id',isset($this->map['package_id'])?$this->map['package_id']:''));?>";</script><?php }?></select>
		</div>
		...
		<div class="form_input">
				<select  name="type" id="type" onchange="if(this.value=='PLUGIN' || this.value=='WRAPPER')$('action_info').style.display='';else $('action_info').style.display='none';if(this.value=='PLUGIN')$('plugin_action_info').style.display='';else $('plugin_action_info').style.display='none';"><?php
					...
					?><?php if(isset($_REQUEST['type']) or isset($this->map['type'])){?><script type="text/javascript">document.getElementById('type').value = "<?php echo addslashes(URL::get('type',isset($this->map['type'])?$this->map['type']:''));?>";</script><?php }?></select>
		</div>
		<div id="action_info" <?php if(URL::get('type')!='PLUGIN' or URL::get('type')!='WRAPPER')echo 'style="display:none"';?>>
			<div id="plugin_action_info" <?php if(URL::get('type')!='PLUGIN')echo 'style="display:none"';?>>
				...
				<select  name="action" id="action"><?php
					...
					?><?php if(isset($_REQUEST['action']) or isset($this->map['action'])){?><script type="text/javascript">document.getElementById('action').value = "<?php echo addslashes(URL::get('action',isset($this->map['action'])?$this->map['action']:''));?>";</script><?php }?></select> </div>
			<?php echo Portal::language('on');?><select  name="action_module_id" id="action_module_id"><?php
					...
					?><?php if(isset($_REQUEST['action_module_id']) or isset($this->map['action_module_id'])){?><script type="text/javascript">document.getElementById('action_module_id').value = "<?php echo addslashes(URL::get('action_module_id',isset($this->map['action_module_id'])?$this->map['action_module_id']:''));?>";</script><?php }?></select>
		...
		<fieldset><legend><?php echo Portal::language('module_table');?></legend>
				<span id="mi_module_table_all_elems" style="text-align:left;">
					<span>
						<span class="multi_input_header"><span style="width:200;"><?php echo Portal::language('table');?></span></span>
						<span class="multi_input_header"><span style="width:20;"><img src="skins/default/images/spacer.gif"/></span></span>
						<br>
					</span>
				</span>
	...
	<ul>
	<?php
					if(isset($this->map['using_pages']) and is_array($this->map['using_pages']))
					{
						foreach($this->map['using_pages'] as $key2=>&$item2)
						{
							if($key2!='current')
							{
								$this->map['using_pages']['current'] = &$item2;?>
	<li><a target="_blank" href="?page=<?php echo $this->map['using_pages']['current']['name'];?>"><?php echo $this->map['using_pages']['current']['name'];?></a> [<a target="_blank" href="<?php echo URL::build_current(array('cmd'=>'delete_block','block_id'=>$this->map['using_pages']['current']['id']));?>">delete block</a>]</li>
					...
					unset($this->map['using_pages']['current']);
					} ?>
	</ul>
</div>
