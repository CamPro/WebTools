...
$this->cond = 'status<>"HIDE" and portal_id="'.Portal::$portal_id.'" and structure_id<>'.ID_ROOT.'';
if(Module::get_setting('category_id'))
{
	if(Module::get_setting('category_id')=='current')
	{
		$cond = 'portal_id="'.Portal::$portal_id.'"';
        $category_id = '';
		...
		if($category_id and $category = DB::select('category',$cond))
		{
            ...
            if(!IDStructure::have_child('category',$root_structure_id)){
				$root_structure_id = IDStructure::parent($root_structure_id);
                $category_name = DB::select('category','structure_id="'.$root_structure_id.'" and portal_id="'.Portal::$portal_id.'"');
