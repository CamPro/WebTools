...
if (! nv_function_exists('nv_block_news_fp_slider')) {
    function nv_block_config_news_fp_slider($module, $data_block, $lang_block)
    {
    ...
    function nv_block_config_news_fp_slider_submit($module, $lang_block)
    {
        global $nv_Request;
        $return = array();
        $return['error'] = array();
    ...
    function nv_block_news_fp_slider($block_config)
    {
        ...
        if (! empty($list)) {
            if (file_exists(NV_ROOTDIR . '/themes/' . $global_config['module_theme'] . '/modules/news/block_fp_slider.tpl')) {
                $block_theme = $global_config['module_theme'];
            } else {
                $block_theme = 'default';
            }
            $xtpl = new XTemplate('block_fp_slider.tpl', NV_ROOTDIR . '/themes/' . $block_theme . '/modules/news');
            ...
            foreach ($list as $l) {
                ...
                $l['hometext_clean'] = nv_clean60($l['hometext_clean'], 120, true);
...
if (defined('NV_SYSTEM')) {
    ...
    if (isset($site_mods[$module])) {
        ...
        $content = nv_block_news_fp_slider($block_config);
    }
}
