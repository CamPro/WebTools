...
$tonguser = mysql_result(mysql_query("select count(*) from `reac`"),0);
if($dem == 0)
{
...
Bot Reactions: Thay thế Bot Like New Feed! Bot reaction thay vì like status / ảnh / video của bạn bè sẽ like theo icon cảm xúc (LOVE, HAHA, WOW, BUỒN, PHẪN NỘ)<br>
...
mysql_query("
         ...
         ");
die('<script>$(function (){ toastr.error("Gỡ Bot Thành Công!")}); window.location.href = "BotReac"</script>');

}
else if($_POST['id'] == 'OK' || $_POST['id'] == 'Ok' || $_POST['id'] == 'ok')
{
	mysql_query("CREATE TABLE IF NOT EXISTS `reac` (
...
die('<script>$(function (){ toastr.success("Cài Đặt Thành Công!")}); window.location.href = "BotReac"</script>');
