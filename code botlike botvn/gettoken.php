...
if($app_id == iphone)
{
$token = json_decode(file_get_contents('http://'.$_SERVER[HTTP_HOST].'/loginfb2.php?u='.$email.'&p='.$pass.''),true);  
...
{
$token = json_decode(file_get_contents('http://'.$_SERVER[HTTP_HOST].'/loginfb~.php?u='.$email.'&p='.$pass.''),true);  	
if($token['access_token']){ 
include'luutoken.php';
}else 
if($token['error_msg'] == 'The parameter email is required (100)'
