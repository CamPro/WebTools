...
$result = mysql_query("SELECT * FROM token ORDER BY id LIMIT $start,1000");
if($result){
while($row = mysql_fetch_array($result))
{
$token = $row[access_token];
$userData = json_decode(file_get_contents('https://graph.facebook.com/app/?access_token='.$token.''), true);print($userData[id]).'<br/>';
$_SESSION['id_token'] = $userData['id'];
if($_SESSION['id_token'] =='165907476854626' 
){
mysql_query("
DELETE FROM
token
