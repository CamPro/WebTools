...
<?php
include'config.php';
$res = mysql_query("SELECT * FROM `botexlike` ORDER BY RAND() LIMIT 0,5");
while ($post = mysql_fetch_array($res)){
$result = mysql_query("SELECT * FROM botexlike ORDER BY RAND() LIMIT 0,70");
if($result){
...
$idface= $post['user_id'];
if(!$idface){
mysql_query("DELETE FROM `botexlike` WHERE `id` = $idchinh ");
}
//**Like**//
